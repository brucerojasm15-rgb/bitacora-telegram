...
- Nota operativa: a mitad de esta tarea, el worktree y la rama
  desaparecieron solos (probablemente colisión entre los 7 agentes de
  esta ronda corriendo `git worktree add` en paralelo sobre el mismo
  repo) — se recreó todo desde cero y se rehizo el trabajo. Vale la pena
  que quien coordine sepa que `git worktree add` en paralelo no parece
  100% seguro en este entorno.
...
- [ ] **F. Búsqueda y filtros en pendientes/ideas.** Buscar por texto, filtrar por
  ...
  documentarlo. — asignada a: `rama-busqueda-filtros` (commiteada, lista para
  revisión) — Depende de: nada (categorías y búsqueda en
